	...
	this.on('JOIN', function JOIN(message) {
		...
		for(var n in channels) {
			...
			if(chan.users[user.nick] == user) {
				console.log("Duplicate join to " + c + " from " + u.nick + ". Ignored.")
			} else {
				chan.join(user, keys[n])
			}
		}
	})
	this.on('UNKNOWN', function UNKNOWN(cmd, params, sender) {
		console.log('Unknown command: '+cmd)
		send(new Message(server, 'NOTICE', 'Auth', 'This server is not finished yet.'))
		send(new Message(server, 'ERROR', 'Closing link'))
		socket.end()
	})
...
Channel.prototype.join = function(user) {
	...
	user.send(this.topic ? new Message(user.server, Codes.RPL_TOPIC, this.topic) : new Message(user.server, Codes.RPL_NOTOPIC, "channel has no topic"))
}
...
function handleLine(l) {
	...
	var msg = new Message(m[1] ? m[1] : undefined, m[2].toUpperCase(), params)
