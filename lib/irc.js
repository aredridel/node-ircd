...
function Daemon(name) {
	...
	this.on('QUIT', function(m) {
		console.log("removing user " + m.sender.full + " from server")
...
Message.prototype.toString = function() {
	...
	o.push(this.command)
	o.push(serializeParams(this)) // FIXME? Might have no params.
	return o.join(' ')
}
...
function Connection(socket, server) {
	...
	this.on('QUIT', function QUIT(message) {
		for(var c in user.channels) {
			console.log("User " + user.nick + " quitting " + user.channels[c].name)
...
Channel.prototype.quit = function(user, message) {
	console.log('Channel ' + this.name + ' got QUIT for ' + user.nick + " with message " + util.inspect(message))
