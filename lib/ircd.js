...
Channel.prototype.join = function(user) {
	...
	if(this.topic) {
		user.send(new Message(user.server, Codes.RPL_TOPIC, nick, this.name, this.topic))
	} else {
		user.send(new Message(user.server, Codes.RPL_NOTOPIC, nick, this.name, "channel has no topic"))
	}
	for(var nick in this.users) {
