...
Channel.prototype.join = function(user) {
	...
	if(this.topic) {
		user.send(new Message(user.server, Constants.RPL_TOPIC, nick, this.name, this.topic))
	} else {
		user.send(new Message(user.server, Constants.RPL_NOTOPIC, nick, this.name, "channel has no topic"))
	}
	for(var nick in this.users) {
		user.send(new Message(user.server, Constants.RPL_NAMREPLY, nick, "=", this.name, nick))
	}
	user.send(new Message(user.server, Constants.RPL_ENDOFNAMES, this.name, "End of NAMES list"))
	return true
}
