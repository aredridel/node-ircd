...
function sendWelcome(server, user) {
	...
	user.send(message.createMessage(server, Constants.RPL_YOURHOST, user.nick, 'Your host is '+server.id))
	user.send(message.createMessage(server, Constants.RPL_CREATED, user.nick, 'This server was created juuuust for you.'))
	user.send(message.createMessage(server, Constants.RPL_MYINFO, user.nick, server.id, 'node-ircd', 'iw', 'o'))
}

function Server(config) {
	net.Server.call(this)
	this.id = config.name ? config.name : 'unconfigured'
	var users_by_full = {}
	var users = {}
	var channels = {}

	this.on('connection', function(s) { this.emit('client', new Connection(s, this)) })
