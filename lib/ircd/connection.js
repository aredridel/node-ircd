...
function Connection(socket, server) {
	...
	this.on('line', function(l) {
		...
		if(checkOrigin(message, user)) {
			if(this.listeners(message.command).length == 0) {
				send(Message.createMessage(server.id, Constants.ERR_UNKNOWNCOMMAND, [user.nick, message.command, 'Unknown command']))
			} else {
...
Connection.prototype.completeRegistration = function completeRegistration(user, server) {
	if(!server.register(user)) {
		this.send(Message.createMessage(server, 'ERROR', 'Something bad happened'))
		socket.end()
	}
